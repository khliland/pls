...
%\date{\today\ (\revision)} % FIXME
\maketitle

%% Setup of Sweave:
\SweaveOpts{width=5,height=5,eps=FALSE} %FIXME: eps=FALSE needed?
...
\begin{figure}
  \begin{center}
\setkeys{Gin}{width=6in}
<<fig=TRUE,echo=FALSE,height=3.5,width=6>>=
par(mar = c(4, 4, 2.5, 1) + 0.1)
plot(gas1, plottype = "coef", ncomp=1:3, legendpos = "bottomleft",
     labels = "numbers", xlab = "nm")
@
...
\begin{figure}
  \begin{center}
\setkeys{Gin}{width=3.7in}
<<fig=TRUE,echo=FALSE,width=3.7,height=3.5>>=
...
The above should work for OpenMPI running under the Slurm queue system.
FIXME: references?  In other situations, one have to specify which machines to
use with, e.g., the \code{-H} switch.
